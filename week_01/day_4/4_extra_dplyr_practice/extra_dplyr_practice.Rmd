...
wine_ratingz <- read_csv("data/wine_ratings.csv")
...
wine_subset %>%
  group_by(country) %>%
  mutate(mean_country = mean(points)) %>%
  select(country, mean_country) %>%
  summarise(country, mean_country)
